...
x = V3 / "outputs" / "FINAL_Xenium_panel_ORBm_BMAp_for_MSGS111.xlsx"
...
base = {
    g.strip()
    for g in (V3 / "outputs" / "xenium_mouse_brain_base_panel.txt").read_text().splitlines()
    if g.strip()
}
...
tf = pd.read_csv(
    V3 / "inputs" / "Jesse_ORB" / "OpioidDependenceDEG_genesets_forHansol090926" / "mPFC_TFDEGs.csv"
